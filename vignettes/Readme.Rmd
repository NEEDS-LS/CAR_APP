...
Feito isso começaremos a tratar possíveis erros que podem surgir com os dados, as *self_intersections* são uma delas. Ocorre quando um poligono é intersectado por ele em algum ponto e é resolvido quando executado um buffer de comprimento 0, geralmente é necessário no objeto contendo o uso do solo que será utilizado. Também é interessante remover os CAR registrado com situação classificada como "cancelado por decisão administrativa", diminuindo consideravelmente o número de sobreposições.
...
A partir daqui executaremos as funções deste pacote, começando pela função *separaTamanho()*. A entrada nesta função é apenas o objeto na qual as informações do CAR estão mantidos e o retorno é uma lista com os tamanhos separados de acordo com o número de módulos fiscais separados nos grupos: micro, entre 1 e 2 módulos, entre 2 e 4 módulos, médias e grandes.
...
Agora podemos executar as funções que, de fato, vão criar os buffers e recortar o uso de solo dentro das áreas definidas para restauração para cada classe de propriedades.Essas funções demandam um poder de processamento consideravel e podem demorar grande quantidades de tempo.

OBS: Caso o município que está sendo analisado não tenha uma das hidrográfias coloque o valor como "NA". 
